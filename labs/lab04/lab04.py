...
def reverse_recursive(lst):
    ...
    # 这里有交换的功能，并且进行了递归！！
    split = n // 2
    fp, bp = lst[:split], lst[split:]
    fp, bp = reverse_recursive(fp), reverse_recursive(bp)
    lst[:split], lst[split:] = bp, fp
    return lst
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance(my_city, city_a) < distance(my_city, city_b) else get_name(city_b)
